...
def extract_cycle_fundamentals(machine_name, t_start_string, t_stop_string, cyclename_must_contain_one_of=[]):
    import PyTimber
...
def add_extracted_intensity_to_SPS_cycles(cycles_SPS):
    import PyTimber
    
    t_start_unix = cycles_SPS['t_stamp_list'][0] - 60.
    t_stop_unix = cycles_SPS['t_stamp_list'][-1] + 60.


    ldb=PyTimber.LoggingDB()

    var = 'SPS.BCTDC.31832:INT_EXTR'
    data = ldb.get(var, t_start_unix, t_stop_unix)[var]
